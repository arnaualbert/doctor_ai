...
def validate_blosum_global_aligment(fasta1: str,fasta2: str,gap_penalty: int) -> bool:
    ...
    print(validator_fasta2_result)
    if validator_fasta1_result == True:
        if validator_fasta2_result == True:
            if int(gap_penalty) < 0:
                return True
            else:
                return 'Gap penalty must be a less than zero'
        else:
            return validator_fasta2_result
    else:
